...
const Monthly = () => {
  ...
  useEffect(async () => {
    let response;
    if (units === "celsius") {
      response = await axios.get(`${API_URL}monthly/celsius`, { params: { CityStateCountry: searchQuery } })
    } else {
      response = await axios.get(`${API_URL}monthly/farenheit`, { params: { CityStateCountry: searchQuery } })
    }
